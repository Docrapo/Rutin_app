let x = 0; /* intercambiador para el nivel 1 */
let y = 0; /* intercambiador para el nivel 2 */
const cant_cardsInterior = document.querySelectorAll(".Card_interior");
/* CONSTANTES del boton de cerrado de seccion */
const a_cierra = document.querySelectorAll("#btn_cierraCard");
const btnCerrar_on = `width: calc(var(--size_menu) / 1.6); height: calc(var(--size_menu) / 1.6); opacity:100; visibility:visible; border:2px solid var(--whiteAppColor);`;

/* ESTO DE DE CARDUNO NO DEBERIA ESTAR ACA*/
function changeCard() {
  if (x == 0) {
...
function cardGrow(inte) {
  ...
  /* $(in).classList = ".card_int_open"; */
  if (y == 0) {
    inte.classList.add("card_int_open");
    console.log(inte.hasChildNodes());
    if (inte.hasChildNodes() == true) {
      inte.querySelector(
        "article > div"
      ).style = `transform: scale(100%); opacity:100; width:100%; height:100%;`;
      inte.querySelector("article > h5").style = `font-size:1.8rem;`;
      document.querySelector(
        ".grafico"
      ).style = `animation: grafico_anim 3s ease-in-out;
  animation-fill-mode: forwards;`;
      document.querySelector(
        ".grafico"
      ).style = `animation: grafico_anim 2s ease-in-out;
  animation-fill-mode: forwards;`;
    }
    y = 1;
  } else {
    inte.classList.remove("card_int_open");
    if (inte.hasChildNodes() == true) {
      inte.querySelector("article > div").style = ``;
      inte.querySelector("article > h5").style = ``;
      document.querySelector(".grafico").style = ``;
    }
    y = 0;
  }
}
/* document.querySelector("#btn_cierraCard").onclick = function () {
  document.querySelector(".Card_interior").classList.remove("card_int_open");
}; */

/* MARK:ClearC
 */
const gridSection = document.querySelectorAll(".item_grid");
function clearCards() {
  document.querySelector(".contenedor_grid").style = ``;
  for (let k = 0; k < gridSection.length; k++) {
    gridSection[k].style = ``;
  }
  for (let l = 0; l < a_cierra.length; l++) {
    a_cierra[l].style = ``;
    x = 1;
  }
  for (let j = 0; j < cant_cardsInterior.length; j++) {
    cant_cardsInterior[j].classList.remove("card_int_open");
    y = 0; /* DEBE ir ya que, al presionar el boton de cerrado, la variable debe volver a cero en caso de que el usuario haya presionado sobre una CARD interna (donde la variable es asignada con y=1 */
  }
  /* SOLO PARA PRUEBA DE ANIMACION EN RUTINA */
  /* document.querySelector("#path_size").style = ``; */
  noverInterior(); /* CIERRA o quita la clase "interior_grid_on" para no ver el contenedor de CARDs */
}
/* function clearCards() {
  ...
  document.querySelector(".contenedor_grid").style = ``;
  for (let k = 0; k < a_cierra.length; k++) {
    a_cierra[k].style = ``;
    x = 1;
  }
  /* SOLO PARA PRUEBA DE ANIMACION EN RUTINA 
...
function clickRutines() {
  ...
  color: #111b2d;`;
    ...
    document.querySelector("#path_size").style = `visibility:hidden;`;
    verInterior();
    for (let i = 0; i < a_cierra.length; i++) {
      a_cierra[i].style = btnCerrar_on;
    }
    x = 1;
  } else {
    x = 0;
...
document.querySelector(".grid_01").onmouseover = function () {
  document.querySelector(".draw_path_off").classList.add("draw_path_on");
};
document.querySelector(".grid_01").onmouseout = function () {
  document.querySelector(".draw_path_off").classList.remove("draw_path_on");
};
/* MARK:CardInter
 */
...
const interior_card = document.querySelectorAll(".interior_grid");

function verInterior() {
  for (let g = 0; g < interior_card.length; g++) {
    interior_card[g].classList.add("interior_grid_on");
  }
  /*document.querySelector(".interior_grid").classList.add("interior_grid_on"); */
}
function noverInterior() {
  for (let g = 0; g < interior_card.length; g++) {
    interior_card[g].classList.remove("interior_grid_on");
  }
  /* document.querySelector(".interior_grid").classList.remove("interior_grid_on"); */
}
